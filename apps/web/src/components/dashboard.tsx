...
export function Dashboard() {
  ...
  const [attendance, setAttendance] = useState<AttendanceState>({
    [demoClasses[0].id]: demoAttendance.filter(
      (item) => item.classId === demoClasses[0].id,
    ),
  });
  ...
  useEffect(() => {
    ...
    if (storedAttendance)
      setAttendance(JSON.parse(storedAttendance) as AttendanceState);
  ...
  async function saveAttendance() {
    ...
    setAttendance((current) => ({
      ...
      [selectedClass.id]: [
        ...
        { classId: selectedClass.id, date: today, absentIds },
      ],
    }));
    setMessage(
      buildAttendanceMessage(selectedClass, selectedStudents, absentIds),
    );
    setNotice(
      t("attendance.saved", {
        present: selectedStudents.length - absentIds.length,
        noun: t("settings.studentNounValue"),
      }),
    );
  }
  ...
  function buildAttendanceMessage(
    ...
    const names = students
      .filter((student) => absent.includes(student.id))
      .map((student) => student.name);
    const absentText = names.length
      ? t("attendance.messageAbsent", { names: names.join(", ") })
      : t("attendance.messageEmpty");
      ...
      <main className="main-content">
        ...
        {view === "renewals" && (
          <Renewals
            ...
            onRenew={recordRenewal}
            onMessage={(student) =>
              share(t("renewals.createMessage") + `: ${student.name}`)
            }
          />
        )}
        {view === "settings" && (
          <SettingsPanel onExport={exportCsv} onReset={resetDemo} />
        )}
      </main>
    </div>
  );
      ...
      <section className="section-block">
        <div className="section-heading">
          <div>
            ...
            <h2>{t("classes.heading")}</h2>
          </div>
          <button className="text-button" onClick={() => onView("classes")}>
            {t("classes.open")} <ChevronRight size={17} />
          </button>
        </div>
...
function ClassCard({
  ...
  return (
    <Button variant="ghost" className="class-card" onClick={onClick}>
      <span>
        <strong>{item.name}</strong>
        <small>{t("classes.schedule", { schedule: item.schedule })}</small>
        <small>{t("classes.branch", { name: item.branch })}</small>
      </span>
...
function Classes({
  ...
  return (
    <>
      ...
      />
      <div className="class-grid wide">
        {classes.map((item) => (
          <div className="class-card-wrap" key={item.id}>
            <ClassCard item={item} onClick={() => onSelect(item.id)} />
            <div className="class-detail">
              <span>{t("classes.teacher", { name: item.teacher })}</span>
              <span>
                {t("classes.count", {
                  count: item.studentIds.filter((id) =>
                    students.some((student) => student.id === id),
                  ).length,
                  noun: t("common.studentNoun"),
                })}
              </span>
            </div>
          </div>
  ...
  return (
    <div className="focus-page">
      <div className="focus-heading">
        <div className="focus-class-select">
          <span className="eyebrow">{t("nav.attendance")}</span>
          <Select
            value={selected.id}
            onChange={(event) => onSelect(event.target.value)}
            aria-label={t("attendance.chooseClass")}
          >
            {classes.map((item) => (
          ...
          <p>
            {selected.schedule} · {selected.studentIds.length}{" "}
            {t("common.studentNoun")}
          </p>
        </div>
      ...
      <Card className="focus-card">
        <div className="focus-hint">
          <Users size={20} />
          <span>{t("attendance.hint", { noun: t("common.studentNoun") })}</span>
        </div>
        <div className="student-list">
          {students.map((student) => {
            ...
            return (
              <button
                key={student.id}
                className={`student-row ${absent ? "is-absent" : ""}`}
      ...
      {message && (
        <Card className="message-card">
          <div className="section-heading">
            <h2>{t("attendance.messageHeading")}</h2>
            <Button
              variant="ghost"
...
function Renewals({
  ...
  onMessage: (student: DemoStudent) => void;
}) {
  return (
    <>
      <PageHeader
        eyebrow={t("nav.renewals")}
        title={t("renewals.heading")}
        description={t("renewals.description")}
      />
      <section className="panel renewal-list">
        {students.length === 0 ? (
            ...
            return (
              <article className="renewal-row" key={student.id}>
                ...
                <div className="row-actions">
                  ...
                  >
                    <Share2 size={16} />
                    {t("renewals.createMessage")}
                  </button>
                  <button
                    className="primary-button compact"
                    onClick={() => onRenew(student.id)}
                  >
                    {t("renewals.record")}
                  </button>
                </div>
    ...
    <>
      ...
      <section className="settings-grid">
        <div className="setting-card">
          <label htmlFor="student-noun">{t("settings.studentNoun")}</label>
          <Select
            id="student-noun"
            defaultValue={t("settings.studentNounValue")}
          >
            <option>{t("settings.studentNounValue")}</option>
            <option>{t("settings.studentNounBaby")}</option>
            <option>{t("settings.studentNounMartial")}</option>
          </Select>
        </div>
        <div className="setting-card">
          <div>
