...
function NavButton({ active, icon, label, onClick }: { active: boolean; icon: React.ReactNode; label: string; onClick: () => void }) {
  return <button className={`nav-button ${active ? 'active' : ''}`} aria-label={label} onClick={onClick}>{icon}<span>{label}</span></button>
}
...
function ClassCard({ item, onClick }: { item: DemoClass; onClick: () => void }) {
  return <button className="class-card" onClick={onClick}><span><strong>{item.name}</strong><small>{t('classes.schedule', { schedule: item.schedule })}</small><small>{t('classes.branch', { name: item.branch })}</small></span><ChevronRight size={19} /></button>
}
